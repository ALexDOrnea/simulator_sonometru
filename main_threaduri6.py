...
BLOCKSIZE = 512
LATENCY = "low"
print("Blocksize fix: 512 samples")
...
raw_queue = queue.Queue(maxsize=8)      # audio thread -> processing thread
data_queue = queue.Queue()     # processing thread -> GUI thread
stop_event = threading.Event()

# ~~~~~BATCH_FACTOR: aliniaza rata de calcul a benzilor de octava + FFT la~~~~~
# rata REALA de desenare (timer GUI = ~30 fps), nu la rata blocurilor audio
# (512 esantioane @ 48kHz inseamna un bloc la fiecare ~10.7ms, adica ~93/sec).
# Inainte, banda de filtre (pana la ~140 filtre de ordin 12 pentru 1/12 octava)
# si FFT-ul se recalculau la FIECARE bloc audio, desi update_gui() oricum
# pastreaza doar ULTIMA valoare primita inainte de a desena - restul erau
# aruncate, nefolosite. Practic 2 din 3 calcule (sau mai mult) se faceau
# degeaba. Batching-ul acumuleaza BATCH_FACTOR blocuri si calculeaza benzile/
# FFT o singura data pe blocul concatenat - matematic IDENTIC cu inainte
# (filtrele sunt LTI, iar starea 'zi' trece corect intre blocuri, la fel ca
# atunci cand eram apelate separat), dar cu de BATCH_FACTOR ori mai putine
# apeluri scipy => mult mai putin overhead. Nivelul rapid (bara/curba dB) si
# Leq raman calculate pe FIECARE bloc, ca sa nu piarda din raspunsul in timp.
BATCH_FACTOR = max(1, round((1.0 / 30.0) * SAMPLE_RATE / BLOCKSIZE))
...
if TIP_PONDERARE == "A-Weighting":
    sos_ponderare = get_a_weighting_filter(SAMPLE_RATE) #coficientii filtrului in format second order sections
    zi_ponderare = sosfilt_zi(sos_ponderare) * 0.0 #conditii initiale filtru
