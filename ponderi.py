...
def trimite_date_live(chunk, chunk_filtrat):
    ...
    data_queue.put((current_time, db_raw, db_filtered, fft_raw, fft_filtered))
...
if ax_db is not None:
    ...
    if SURSA_OPT == "1":
        ...
        ax_db.set_title(
            f"Nivel live: nefiltrat vs {TIP_FILTRU}{titlu_frecv_str} - Mod: {MODE}"
        )
    else:
        ax_db.set_xlim(0, 10)
        ax_db.set_title(
            f"Nivel LIVE microfon: nefiltrat vs {TIP_FILTRU}{titlu_frecv_str} - Mod: {MODE}"
        )
...
if ax_fft is not None:
    ...
    # Adăugăm linia verticală DOAR dacă există frecvență de tăiere
    ...
    ax_fft.legend()
    ax_fft.set_title(
        f"FFT in timp real: nefiltrat vs {TIP_FILTRU}{titlu_frecv_str}"
    )
