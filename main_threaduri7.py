...
# rata REALA de desenare (timer GUI, vezi GUI_REFRESH_MS), nu la rata
# blocurilor audio (256 esantioane @ 48kHz inseamna un bloc la fiecare
# ~5.3ms, adica ~187/sec). Inainte, banda de filtre (pana la ~140 filtre de
# ordin 12 pentru 1/12 octava) si FFT-ul se recalculau la FIECARE bloc audio,
# desi update_gui() oricum pastreaza doar ULTIMA valoare primita inainte de a
# desena - restul erau aruncate, nefolosite. Batching-ul acumuleaza
# BATCH_FACTOR blocuri si calculeaza benzile/FFT o singura data pe blocul
# concatenat - matematic IDENTIC cu inainte (filtrele sunt LTI, iar starea
# 'zi' trece corect intre blocuri, la fel ca atunci cand erau apelate
# separat), dar cu de BATCH_FACTOR ori mai putine apeluri scipy. Nivelul
# rapid (bara/curba dB) si Leq raman calculate pe FIECARE bloc, ca sa nu
# piarda din raspunsul in timp.
BATCH_FACTOR = max(1, round((GUI_REFRESH_MS / 1000.0) * SAMPLE_RATE / BLOCKSIZE))
