...
print("\n~~~~~CONFIGURARE BLOCKSIZE~~~~~")
print("Numarul de esantioane procesate per callback audio.")
print("Gol/0 = lasa PortAudio sa aleaga automat (recomandat daca nu esti sigur).")
print("Valori tipice: 64, 128, 256, 512, 1024 (mai mic = latenta mai mica, dar risc mai mare de xrun-uri).")
try:
    _blocksize_in = input("Blocksize (esantioane) > ").strip()
    BLOCKSIZE = int(_blocksize_in) if _blocksize_in else 0
    if BLOCKSIZE < 0:
        raise ValueError
except ValueError:
    print("Valoare invalida. se foloseste 0 (automat)")
    BLOCKSIZE = 0

print("\n~~~~~CONFIGURARE LATENCY~~~~~")
print("1 low  (latenta minima, risc mai mare de xrun-uri)")
print("2 high (latenta mai mare, mai stabil)")
print("3 valoare numerica proprie (secunde, ex: 0.01)")
_latency_opt = input("> ").strip().lower()
if _latency_opt in ("1", "low", ""):
    LATENCY = "low"
elif _latency_opt in ("2", "high"):
    LATENCY = "high"
else:
    try:
        LATENCY = float(_latency_opt.replace(",", "."))
    except ValueError:
        print("Valoare invalida. se foloseste 'low'")
        LATENCY = "low"
...
def update_gui():
    ...
    if PEAK_MODE:
        ...
        if plot_db is not None:
            ...
            curve_db_raw.setData(list(x_data), list(y_db_raw))
            curve_db_filtered.setData(list(x_data), list(y_db_filtered))
