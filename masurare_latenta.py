...
# Modificari:
# - blocksize fix 512 / 256
# - latency low stabil
# - queue limitata pentru evitarea acumularii de delay
# - procesare audio float32
# - GUI 60 FPS
# - masurare live a timpului de raspuns la spike (bataie din palme)
# ==========================================================
...
# --- VARIABILE PENTRU MASURAREA TIMPULUI DE RASPUNS (SPIKE) ---
PRAG_SPIKE_DB = -25.0 + CALIBRARE_DB
spike_in_desfasurare = False
t_start_spike = 0.0
peak_spike_db = NIVEL_PODEA
t_peak_spike = 0.0
esantioane_analizate_spike = 0
MAX_ESANTIOANE_CAUTARE = int(0.20 * SAMPLE_RATE)  # Fereastra de 200 ms

def proceseaza_chunk(chunk, chunk_ponderat, pointer_esantion):
    global zi_nivel_raw, zi_nivel_filt, _batch_count
    global spike_in_desfasurare, t_start_spike, peak_spike_db, t_peak_spike, esantioane_analizate_spike
    ...
    if PEAK_MODE:
        ...
        # Monitorizare spike pentru modul Peak
        acum = time.perf_counter()
        nivel_curent = db_raw
        if not spike_in_desfasurare and nivel_curent > PRAG_SPIKE_DB:
            spike_in_desfasurare = True
            t_start_spike = acum
            peak_spike_db = nivel_curent
            t_peak_spike = acum
            esantioane_analizate_spike = 0

        if spike_in_desfasurare:
            esantioane_analizate_spike += len(chunk)
            if nivel_curent > peak_spike_db:
                peak_spike_db = nivel_curent
                t_peak_spike = acum

            if esantioane_analizate_spike >= MAX_ESANTIOANE_CAUTARE:
                timp_pana_la_peak_ms = (t_peak_spike - t_start_spike) * 1000.0
                durata_buffer_ms = (len(chunk) / SAMPLE_RATE) * 1000.0
                print("\n" + "=" * 55)
                print(" [SPIKE DETECTAT - BATAIE DIN PALME]")
                print(f"  * Nivel Maxim:            {peak_spike_db:6.1f} dB")
                print(f"  * Timp raspuns la varf:   {timp_pana_la_peak_ms:6.2f} ms")
                print(f"  * Latenta unui chunk DSP: {durata_buffer_ms:6.2f} ms")
                print("=" * 55 + "\n")
                spike_in_desfasurare = False
    ...
    # Monitorizare spike pentru modurile Fast/Slow
    acum = time.perf_counter()
    nivel_curent = db_raw
    if not spike_in_desfasurare and nivel_curent > PRAG_SPIKE_DB:
        spike_in_desfasurare = True
...
if SURSA_OPT == "2" and not PEAK_MODE:
    ...
    _MAXLEN_LIVE = int(_puncte_pe_secunda * 15)
