...
# - GUI 30 FPS
# - calibrare SPL actualizabila live din interfata (buton + camp text)
# ==========================================================
...
def actualizeaza_calibrare(noua_valoare):
    """Actualizeaza din mers constanta de calibrare si pragurile derivate din
    ea (NIVEL_PODEA/NIVEL_PLAFON). Apelata din threadul GUI (Qt slot), deci
    ruleaza mereu pe main thread - niciodata din threadul audio sau de
    procesare."""
    ...
    with calibrare_lock:
        ...
        NIVEL_PODEA = -120.0 + CALIBRARE_DB
        NIVEL_PLAFON = 20.0 + CALIBRARE_DB
    # dupa o recalibrare, vechile valori de peak/Leq au fost calculate cu alt
    # offset si nu mai sunt comparabile cu ce urmeaza - le resetam, ca la un
    # sonometru real dupa o recalibrare cu calibratorul acustic.
    reseteaza_peak_hold()
    reseteaza_leq()
...
STIL_CAMP_CALIBRARE_NORMAL = (
    "QLineEdit { background-color: #222; color: white; border: 1px solid #555; padding: 4px; }"
)
STIL_CAMP_CALIBRARE_EROARE = (
    "QLineEdit { background-color: #222; color: white; border: 1px solid #e74c3c; padding: 4px; }"
)

def creeaza_panou_calibrare():
    """Panou mic cu: eticheta 'in uz' (constanta curenta), un camp text pentru
    valoarea noua si un buton 'Aplica'. Returneaza layout-ul + widget-urile ca
    sa poata fi conectate la un handler in exteriorul functiei."""
    panou = QtWidgets.QVBoxLayout()

    eticheta_titlu = QtWidgets.QLabel("Calibrare SPL")
    eticheta_titlu.setStyleSheet("color: white; font-size: 12px; font-weight: bold;")
    eticheta_titlu.setAlignment(QtCore.Qt.AlignCenter)

    eticheta_curenta = QtWidgets.QLabel(f"In uz: {CALIBRARE_DB:+.2f} dB")
    eticheta_curenta.setStyleSheet("color: #2ecc71; font-size: 12px; font-weight: bold;")
    eticheta_curenta.setAlignment(QtCore.Qt.AlignCenter)

    camp_valoare = QtWidgets.QLineEdit()
    camp_valoare.setPlaceholderText("noua constanta (dB)")
    camp_valoare.setStyleSheet(STIL_CAMP_CALIBRARE_NORMAL)
    camp_valoare.setAlignment(QtCore.Qt.AlignCenter)

    buton_aplica = QtWidgets.QPushButton("Aplica")
    buton_aplica.setStyleSheet(
        "QPushButton { background-color: #333; color: white; padding: 6px; font-size: 11px; }"
        "QPushButton:hover { background-color: #555; }"
        "QPushButton:pressed { background-color: #777; }"
    )

    panou.addWidget(eticheta_titlu)
    panou.addWidget(eticheta_curenta)
    panou.addWidget(camp_valoare)
    panou.addWidget(buton_aplica)
    return panou, camp_valoare, buton_aplica, eticheta_curenta

def creeaza_handler_calibrare(camp_valoare, eticheta_curenta):
    """Fabrica de handler: intoarce o functie fara argumente (potrivita pentru
    clicked/returnPressed) care citeste textul din camp_valoare, il aplica prin
    actualizeaza_calibrare() si actualizeaza eticheta + axele Y ale graficelor
    deja create. Facuta ca fabrica pentru ca ambele moduri (PEAK_MODE si
    normal) au propriile lor instante de camp/eticheta."""
    def handler():
        text = camp_valoare.text().strip().replace(",", ".")
        if not text:
            return
        try:
            valoare_noua = float(text)
        except ValueError:
            camp_valoare.setStyleSheet(STIL_CAMP_CALIBRARE_EROARE)
            return

        actualizeaza_calibrare(valoare_noua)

        eticheta_curenta.setText(f"In uz: {CALIBRARE_DB:+.2f} dB")
        camp_valoare.clear()
        camp_valoare.setStyleSheet(STIL_CAMP_CALIBRARE_NORMAL)

        # graficele (daca exista) au fost create cu vechile NIVEL_PODEA/NIVEL_PLAFON -
        # le realiniem la noile praguri; restul (bare, culori, clip) se
        # actualizeaza deja automat la fiecare frame, pentru ca citesc globalele
        # curente.
...
if PEAK_MODE:
    ...
    panou_calibrare, camp_calibrare, buton_calibrare, eticheta_calibrare_curenta = creeaza_panou_calibrare()
    handler_calibrare = creeaza_handler_calibrare(camp_calibrare, eticheta_calibrare_curenta)
    buton_calibrare.clicked.connect(handler_calibrare)
    camp_calibrare.returnPressed.connect(handler_calibrare)
    ...
    layout_vertical.addLayout(panou_calibrare)
    ...
    layout_meter.addLayout(panou_calibrare)
...
try:
    ...
    print(f"Constanta de calibrare in uz la finalul rularii: {CALIBRARE_DB:+.2f} dB")
