...
# Callback duplex pentru microfon:
# preia semnalul de la microfon, il filtreaza si il trimite imediat la casti.
def microphone_duplex_callback(indata, outdata, frames, time_info, status):
    global play_pointer
    ...
    # Semnalul auzit la iesire este cel filtrat.
    # Limitarea evita valori in afara intervalului audio [-1, 1].
    outdata[:, 0] = np.clip(chunk_filtrat, -1.0, 1.0)
...
try:
    if SURSA_OPT == "1":
        ...
        print(
            f"Se preia microfonul, se reda in casti semnalul filtrat {TIP_FILTRU} "
            f"la {CUTOFF_HZ:g} Hz si se afiseaza comparatia live..."
        )
        print(
            "ATENTIE: foloseste casti, nu difuzoarele laptopului, pentru a evita microfonia."
        )
        stream = sd.Stream(
            samplerate=SAMPLE_RATE,
            channels=(1, 1),
            dtype="float32",
            latency="low",
            callback=microphone_duplex_callback,
        )
