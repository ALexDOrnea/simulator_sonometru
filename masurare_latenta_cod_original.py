...
t_start_spike = 0.0
peak_spike_db = -120.0 + CALIBRARE_DB
t_peak_spike = 0.0
...
def trimite_date_live(chunk, chunk_ponderat):
    ...
    if PEAK_MODE:
        ...
        # Monitorizare spike pe modul Peak
        ...
        # Monitorizare spike pe modurile Fast/Slow
        acum = time.perf_counter()
        nivel_curent = db_raw
        if not spike_in_desfasurare and nivel_curent > PRAG_SPIKE_DB:
            spike_in_desfasurare = True
...
def actualizeaza_bara(bara, eticheta_valoare, eticheta_peak, valoare_db, valoare_peak_db):
    interval_bara = (20.0 + CALIBRARE_DB) - (-120.0 + CALIBRARE_DB)
