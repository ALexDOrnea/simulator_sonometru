...
WAV_PATH = ""
MONITORING_OPT = False

if SURSA_OPT == "1":
    WAV_PATH = input("insert file path: ")
elif SURSA_OPT == "2":
    # Întrebăm utilizatorul dacă dorește monitorizare în timp real
    mon_input = input("Vrei monitorizare audio live in boxe? (y/n/da/nu): ").strip().lower()
    if mon_input in ["y", "yes", "da"]:
        MONITORING_OPT = True
        print("Monitorizare activata! Sunetul de la microfon va fi trimis in output.")
    else:
        print("Monitorizare dezactivata (fara sunet in boxe).")
else:
    print("Sursa inexistenta.")
    sys.exit()
...
# Callback pentru microfon (FARA monitorizare)
...
# Callback pentru microfon (CU monitorizare - Full Duplex)
def record_callback_with_monitoring(indata, outdata, frames, time_info, status):
    global play_pointer
    if status:
        print(status, file=sys.stderr)
    ...
    # Trimitem sunetul captat direct în output (boxe)
    outdata[:, 0] = chunk
...
try:
    if SURSA_OPT == "1":
        ...
        if MONITORING_OPT:
            # Folosim sd.Stream (Full Duplex: 1 input channel, 1 output channel) pentru monitorizare
            stream = sd.Stream(samplerate=SAMPLE_RATE, channels=(1, 1), callback=record_callback_with_monitoring)
        else:
            # Folosim sd.InputStream (doar intrare) când nu vrem monitorizare
